"""Transfer-learning classifiers: DenseNet121 / EfficientNet-B0.

Input contract (the single most important detail here): the model takes RGB
float32 in **[0, 255]**.  Each backbone's own normalisation is baked in as the
first layers, so the tf.data pipeline, the Grad-CAM code, the Streamlit demo
and the TFLite model all feed the network the same thing.

Two named layers are the Grad-CAM contract:
  * `feature_map` - the last convolutional feature map (7x7xC at 224x224 input)
...
def _backbone(name: str, img_size: int) -> keras.Model:
...
def build_model(backbone: str = "densenet121", img_size: int = 224,
    ...
    if backbone == "densenet121":
        ...
        # EfficientNet ships its own Rescaling + Normalization inside the graph,
        # so it must be fed raw [0, 255] values.
        x = inputs

    base = _backbone(backbone, img_size)
    base.trainable = False
    # training=False keeps BatchNorm in inference mode for the whole run - the
    # standard fine-tuning recipe for a small dataset with batch size 32.
